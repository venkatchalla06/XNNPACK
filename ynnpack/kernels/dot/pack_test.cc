...
TEST(pack_a, aligned_blocks) {
  ...
  packer_a p(elem_size * 8, block_m, tile_k);
  p.pack(m, k, k * elem_size, input.data(), output_ko_stride, output_mo_stride,
         output.data());
...
TEST(pack_a, unaligned_padding) {
  ...
  pack_a(m, k, block_m, tile_k, elem_size, k * elem_size, input.data(),
         output_ko_stride, output_mo_stride, output.data());
